...
function typecheck_from_file(file::AbstractString)
    ...
    typecheck_from_string_wrapper(Expr(:block, ast.args...),file_content,false,false)
end
function typecheck_from_file_detailed(file::AbstractString)
    file_content = read(file, String)
    ast = Meta.parseall(file_content, filename = file)
    println("read file $file")
    typecheck_from_string_wrapper(Expr(:block, ast.args...),file_content,true,false)
end
function typecheck_from_file_debug(file::AbstractString)
    file_content = read(file, String)
    ast = Meta.parseall(file_content, filename = file)
    println("read file $file")
    typecheck_from_string_wrapper(Expr(:block, ast.args...),file_content,true,true)
end
...
The typechecking result will be printed to the REPL. It will be the inferred type of the last statement in the file.
"""
function typecheck_from_string(term::String)
    ast = Meta.parse("begin $term end")
    typecheck_from_string_wrapper(ast,term,false,false)
end
function typecheck_from_string_detailed(term::String)
    ast = Meta.parse("begin $term end")
    typecheck_from_string_wrapper(ast,term,true,false)
end
function typecheck_from_string_debug(term::String)
    ast = Meta.parse("begin $term end")
    typecheck_from_string_wrapper(ast,term,true,true)
end

function typecheck_from_string_wrapper(ast::Expr, rawsource::String, bShowDetailedInfo::Bool, bShowDebugInfo::Bool)
    ast = expand_includes(ast)
