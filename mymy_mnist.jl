...
function feedforward(network, x)
  layer_output = [x]
  for l in network
      layer_output = [layer_output..., l.activation(l.weights' * layer_output[end] + l.bias)]
  end
  return layer_output
end
...
function gradient(network, x, y)
  # feedforward
  layer_output = feedforward(network, x)
  ...
  delta = (y - layer_output[end]) # derivative of quadratic loss
  nabla_b[end] = delta
  nabla_w[end] = layer_output[end-1] * delta'

  for l in 2:length(network)-1 # assume there's an input layer that we don't want to update the weights of
      sp = network[end-l+1].derivative(layer_output[end-l+1])
      delta = (network[end-l+2].weights * delta) .* sp

      nabla_b[end-l+1] = delta
      nabla_w[end-l+1] = (delta * layer_output[end-l]')'
  end

  return (nabla_b, nabla_w, layer_output[end])
end
...
function test(network, features, labels)
   # accuracy for batch status output
	...
	for k in 1:length(features)
	   layer_output = feedforward(network, 6 .* features[k] .- 3)
